...
// Helper to create a properly-formed ChatMessage
const createMessage = (role: ChatMessage['role'], content: string, needsFeedback?: boolean): ChatMessage => ({
  role,
  content,
  needsFeedback,
  timestamp: new Date(),
...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([createMessage('assistant', 'Welcome! How can I help you with the Jazeera Airways API today?')]);
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const REDACTED = (satisfied: boolean, index: number) => {
    const updatedMessages = [...messages];
    updatedMessages[index].needsFeedback = false;
    updatedMessages.push(createMessage('user', satisfied ? "Yes, that was helpful" : "No, I need more help"));

    if (!satisfied) {
      updatedMessages.push(createMessage('assistant', "I'm sorry the answer wasn't helpful. Could you please rephrase your question or provide more details so I can better assist you?", true));
    }
  ...
  setMessages(prev => [...prev, createMessage('user', userMessage)]);
    ...
    try {
      ...
      if (faqResponse) {
        setMessages(prev => [...prev, createMessage('assistant', faqResponse, true)]);
        setIsLoading(false);
        return;
      }
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button
          variant="default"
          className="fixed bottom-4 right-4 h-14 px-6 shadow-lg bg-blue-600 hover:bg-blue-700 flex items-center gap-2 rounded-full REDACTED transition-all duration-300 border-2 border-white/20"
        >
