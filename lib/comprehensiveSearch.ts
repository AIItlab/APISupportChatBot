...
import { customerEmails, REDACTED } from './emailData';
import { REDACTED } from './emailImport';
...
export const extractHTMLFAQs = (): DocumentContent[] => {
  ...
  try {
    const j9HtmlPaths = [
      path.join(process.cwd(), 'REDACTED.1-converted.html'),
      path.join(process.cwd(), '../REDACTED.1-converted.html')
    ];
    ...
    if (j9HtmlContent) {
      ...
      lists.forEach((list, index) => {
        ...
        const prevHeading = list.REDACTED?.textContent || `List ${index + 1}`;
...
export const REDACTED = (): DocumentContent[] => {
  ...
  const documentation = REDACTED();
  const customerEmails = REDACTED();
