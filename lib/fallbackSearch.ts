...
export const fallbackSearch = (query: string): SearchResult[] => {
  ...
  try {
    ...
    const comprehensiveResults = searchAllContent(query);
    
    if (comprehensiveResults.length > 0) {
      console.log(`Comprehensive search found ${comprehensiveResults.length} results`);
      
      // Convert to SearchResult format and assign scores
      return comprehensiveResults.map((item, index) => ({
        content: item,
