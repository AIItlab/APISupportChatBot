...
type PineconeCompatibleMetadata = Record<string, string | number | boolean>;
    ...
    const vectors = await Promise.all(
      knowledgeBase.map(async (item: FAQ | DocumentationSection) => {
        try {
          ...
          const baseMetadata: PineconeCompatibleMetadata = {
            id: item.id,
            type: 'question' in item ? 'faq' : 'doc'
          };

          const metadata: PineconeCompatibleMetadata = 'question' in item 
            ? {
                ...baseMetadata,
